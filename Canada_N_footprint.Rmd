...
N_recommended_rates <- read_csv("Data/Food/fertilizer/recommended_fertilizer.csv", skip = 2)%>%
...
VN_sensitivity_PROV <- left_join(food_consum, VNF_CAN, by =c("food_group"="Food group"))%>%
  ...
  mutate(`VN USA` = case_when(`food_group` == "Oilseeds" ~ `adjusted consumed kg/cap/year` * `VNF kgN/kg weighted`,
                                TRUE ~ `adjusted consumed kg/cap/year` * `VNF kg N / kg food USA`),
         `VN CAN weighted` = `adjusted consumed kg/cap/year` * `VNF kgN/kg weighted`,
         `VN provincial` = case_when(is.na(`VNF kgN/kg`) ~ `adjusted consumed kg/cap/year` * `VNF kgN/kg weighted`,
                                     TRUE ~ `adjusted consumed kg/cap/year` * `VNF kgN/kg`))%>%
         
  rename(Subsector = food_group)%>%
  select(GEO, `N footprint sector`, Subsector, Population, `VN USA`, `VN CAN weighted`, `VN provincial`)%>%
  pivot_longer(cols = starts_with("VN"), names_to = "VN", values_to = "N kg/cap/year")%>%
  group_by(GEO,`N footprint sector`, Subsector, VN, Population)%>%
  summarise(`N kg/cap/year` = sum(`N kg/cap/year`))%>%
  filter(GEO !="Canada")%>%
  mutate(`N kg/year` = `N kg/cap/year` * Population)

VN_sensitivity_CAN <- VN_sensitivity_PROV%>%
  group_by(`N footprint sector`, Subsector, VN)%>%
...
VN_sensitivity <- VN_sensitivity_PROV%>%
  ...
  mutate(VN = factor(VN, levels = c("VN provincial", "VN CAN weighted", "VN USA")))

ggplot(VN_sensitivity, aes(fill = `N footprint sector`, y = `N kg/cap/year`, x = `VN`))+
  geom_bar(stat="identity") + 
  facet_wrap(~factor(`GEO`, levels=c("Canada", "British Columbia", "Alberta", "Saskatchewan", "Manitoba", "Ontario", "Quebec", "New Brunswick", "Nova Scotia", "Prince Edward Island", "Newfoundland and Labrador")))+
  #geom_text(aes(label = round(`N kg/cap/year`, 1)), position = position_stack(vjust= 0.5), size = 3)+
...
sludge is typically burned or added to landfill in Canada (so still released as Nr). Potential recycling 25%
...
N_wastewater <-  read_csv("Data/Food/wastewater/wastewater.csv", skip = 1)%>%
  ...
  ungroup%>%
  mutate(`%N removal low` = case_when(`Population by treatment type` == "No treatment" ~ 0.05,
                                  `Population by treatment type` == "Primary treatment" ~ 0.05,
                                  `Population by treatment type` == "Secondary treatment" ~ 0.08,
                                  `Population by treatment type` == "Secondary treatment with additional phosphorus removal" ~ 0.08,
                                  `Population by treatment type` == "Tertiary treatment" ~ 0.20),
         `%N removal high` = case_when(`Population by treatment type` == "No treatment" ~ 0.10,
                                  `Population by treatment type` == "Primary treatment" ~ 0.10,
                                  `Population by treatment type` == "Secondary treatment" ~ 0.15,
                                  `Population by treatment type` == "Secondary treatment with additional phosphorus removal" ~ 0.15,
                                  `Population by treatment type` == "Tertiary treatment" ~ 0.35))%>%
  filter(year %in% years)%>%
  group_by(GEO, VALUE, `Population by treatment type`, `%N removal high`, `%N removal low`)%>%
  summarise(percent_pop = mean(percent_pop))
  
N_wastewater_PROV <- left_join(food_consum, N_wastewater)%>%
  ...
  mutate(`N wastewater loss kg/cap/year high` = `adjusted consumed N kg/cap/year` * (percent_pop) * (1-`%N removal high`),
         `N wastewater loss kg/cap/year low` = `adjusted consumed N kg/cap/year` * (percent_pop) * (1-`%N removal low`))%>%
  group_by(GEO)%>%
  summarise(`High` = sum(`N wastewater loss kg/cap/year high`),
            `Low` = sum(`N wastewater loss kg/cap/year low`))%>%
  pivot_longer(cols = !GEO, names_to = "Treatment removal", values_to = "N kg/cap/year")%>%
  mutate(`N footprint sector` = "Wastewater Treatment", "Subsector" = "Wastewater Treatment")%>%
  left_join(prov_pop)%>%
  mutate(`N kg/year` = `N kg/cap/year` * Population)
 
N_wastewater_CAN <- N_wastewater_PROV%>%
  group_by(`Treatment removal`)%>%
...
N_wastewater_Total <- bind_rows(N_wastewater_PROV, N_wastewater_CAN)%>%
  select(!(Population))%>%
  mutate(GEO = factor(GEO, levels = c("Canada", "British Columbia", "Alberta", "Saskatchewan", "Manitoba", "Ontario", "Quebec", "New Brunswick", "Nova Scotia", "Prince Edward Island", "Newfoundland and Labrador")),
         `Treatment removal` = factor(`Treatment removal`, levels = c("Low", "High")))

ggplot(N_wastewater_Total, aes(fill = `Treatment removal`, y = `N kg/cap/year`, x = `Treatment removal`))+
  geom_bar(stat="identity") + 
  ...
  labs(caption = "Canadian wastewater N footprints, low N removal vs high N removal assumptions (N kg/cap/year)")+
  coord_flip()+
  scale_fill_brewer(palette="Paired")+
  theme_classic()+
  theme(axis.text.x = element_text(size=8, angle = 90, hjust = 1, vjust = 0.5),
...
N_footprint <- bind_rows(N_wastewater_Total, N_energy)%>%
  ...
  filter(`Treatment removal` == "Low" | is.na(`Treatment removal`))%>%
  filter(`VN` == "VN provincial"| is.na(`VN`))%>%
...
ggsave("Results_Images/crop_VNFs.pdf")

ggplot(VNF_meat, aes(fill = `GEO`, y = `VNF kgN/kg`*1000, x = GEO))+
  geom_bar(position="stack", stat="identity")+
  facet_wrap(~factor(`Food group`, levels=c("Milk", "Eggs", "Chicken", "Pork", "Beef")), scales = "free_x")+ #fish not included because there is only one value
...
ggsave("Results_Images/animal_VNFs.pdf")  
...
ggplot(subset(N_footprint, GEO != "Canada"), aes(fill = `N footprint sector`, y = `N kg/year`/1000000, x = reorder(GEO, - `N kg/year`)))+
  geom_bar(stat="identity") +
  scale_fill_brewer(palette="Paired")+
