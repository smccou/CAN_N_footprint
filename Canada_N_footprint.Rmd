...
---
Ongoing notes:
the fertilizer application on Tame hay makes a reasonably big difference in VNFs
...
# for source/sector descriptors http://publications.gc.ca/collections/collection_2021/eccc/En81-30-2019-eng.pdf
APEI_energy <- read_csv("Data/fossil_fuels/APEI_inventory.csv", skip = 1)%>%
  ...
  mutate(Source = case_when(Source == "Ore and Mineral Industries" ~ "Other Industry",
                            Source == "Oil and Gas Industry" ~ "Oil and Gas Industry",
                            Source == "Electric Power Generation (Utilities)" ~ "Energy Generation",
                            Source == "Manufacturing" ~ "Other Industry",
                            (Source == "Transportation and Mobile Equipment" & Sector != "Marine Transportation")  ~ "Transport",
                            Sector == "Commercial and Institutional Fuel Combustion" ~ "Commercial & Residential",
                            Sector == "Residential Fuel Combustion" ~ "Commercial & Residential"))%>% #Incineration and waste captured in Wastewater, paints, dust, and fire sources minimal
  replace_na(list(`NOX (t)`= 0, `NH3 (t)` = 0))%>%
  #convert NOx weights to N, NOx reported as NO2 equivalent
  #take into account oil export
  mutate(N_kg = case_when(Source == "Oil and Gas Industry" ~ ((`NOX (t)` * 1000 * 14.0067 / 46.0055) + (`NH3 (t)` * 1000 * 14.0067 / 17.031)) * 0.15,
                          TRUE ~ ((`NOX (t)` * 1000 * 14.0067 / 46.0055) + (`NH3 (t)` * 1000 * 14.0067 / 17.031))))%>%  
...
#see GHG executive summary file for explanation of categories http://publications.gc.ca/collections/collection_2020/eccc/En81-4-2018-1-eng.pdf
GHG_energy <- read_csv("Data/fossil_fuels/GHG_inventory.csv", skip = 1)%>%
  ...
  mutate(Source = case_when(Category == "Public Electricity and Heat Production" ~ "Energy Generation",
                            ...
                            Category == "Commercial and Institutional" ~ "Commercial & Residential",
...
VN_sensitivity_PROV <- left_join(food_consum, VNF_CAN, by =c("food_group"="Food group"))%>%
  ...
  mutate(`VNF USA` = case_when(`food_group` == "Oilseeds" ~ `adjusted consumed kg/cap/year` * `VNF kgN/kg weighted`,
  ...
  rename(Subsector = food_group)%>%
  select(GEO, `N footprint sector`, Subsector, Population, `VNF USA`, `VNF CAN weighted`, `VNF provincial`)%>%
...
VN_sensitivity <- VN_sensitivity_PROV%>%
  ...
  mutate(VNF = factor(VNF, levels = c("VNF provincial", "VNF CAN weighted", "VNF USA")),
         GEO = case_when(GEO == "Prince Edward Island" ~ "PEI",
                   GEO == "Newfoundland and Labrador" ~ "NFLD",
                   TRUE ~ GEO))
...
ggplot(VN_sensitivity, aes(fill = `N footprint sector`, y = `N kg/cap/year`, x = `VNF`))+
  ...
  facet_wrap(~factor(`GEO`, levels=c("Canada", "British Columbia", "Alberta", "Saskatchewan", "Manitoba", "Ontario", "Quebec", "New Brunswick", "Nova Scotia", "PEI", "NFLD")))+
...
APEI_wastewater <- read_csv("Data/fossil_fuels/APEI_inventory.csv", skip = 1)%>%
  ...
  filter(Year %in% years)%>%
  group_by(GEO)%>%
  summarise(`NH3 NOx loss Nkg/year` = mean(`NH3 NOx loss Nkg/year`))
...
N_wastewater_PROV <- left_join(N_aqueous_wastewater,APEI_wastewater)%>%
  ...
  mutate(`N recyclable kg/year` = `N inffluent N kg` - `N aqueous loss N kg` - `N20 loss N kg` - `N2 N kg`,
         ...
         `High` = `N aqueous loss N kg` + `N20 loss N kg` + `50% recycled`, 
         `Low` = `N aqueous loss N kg` + `N20 loss N kg` + `25% recycled`)%>%
...
ggsave("Results_Images/wastewater_sensitivity_graph.pdf")
...
N_footprint <- bind_rows(N_wastewater_Total, N_energy)%>%
  ...
  mutate(GEO = case_when(GEO == "Prince Edward Island" ~ "PEI",
         ...
         Subsector = fct_relevel(Subsector, "Beef", "Pork", "Chicken", "Eggs", "Milk", "Fish", "Legumes", "Oilseeds", "Grains", "Fruits", "Vegetables", "Roots", "Wastewater Treatment", "Energy Generation", "Oil and Gas Industry", "Transport", "	
Other Industry", "Commercial & Residential"),
...
write_csv(VNF_graph, "Data/Food_VNFs.csv")
...
ggsave("Results_Images/VNF_graph.pdf")  
...
ggsave("Results_Images/N_print_prov_sectors_cap_graph.pdf")
...
ggsave("Results_Images/N_print_prov_sectors_percent_graph.pdf")
