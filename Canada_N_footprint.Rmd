...
prov_pop <- read_excel("Data/Data.xlsx", sheet = "Prov_pop", skip = 1)%>%
...
APEI_energy <- read_excel("Data/Data.xlsx", sheet = "APEI_inventory", skip = 1)%>%
...
GHG_energy <- read_excel("Data/Data.xlsx", sheet = "GHG_inventory", skip = 1)%>%
...
fruit_list = c("apples", "blueberries", "grapes", "peaches", "pears", "plums and prune plums", "raspberries", "strawberries", "nectarines", "sour cherries", "sweet cherries", "cranberries")
...
#removed "saskatoon berries" only grown is saskatchewan, no apricots (under 100 ha), black berries, currants, haskap
...
#Field Crops
field_crop_survey <- read_excel("Data/Data.xlsx", sheet = "field_crop_survey", skip = 1)%>%
...
N_BNF <- mutate(crop_prod, key = case_when(StatCan_name %in% vegetable_list ~ "Vegetables",
  ...
  mutate(yield_DM_kg = yield_kg * ((1-(`H20 content (%w/w)`/100))),
         ...
         residue_DM_kg = case_when(key %in% c("Berries and grapes", "Tree fruit") ~ ((yield_DM_kg / HI) - yield_DM_kg) * 0.1, #takes in to account 10 yr lifespan of trees/shrubs
                                StatCan_name == "Tame hay" ~ ((yield_DM_kg / HI) - yield_DM_kg), #takes in to account 5 yr lifespan of hay
...
#Partial nutrient balance n yield/n applied
...
food_processing_waste <- read_excel("Data/Food/Waste/food_loss_USDA.xlsx", range = "A1:D12")
...
N_crop_process <- mutate(N_field_loss, key_process = case_when(StatCan_name %in% cereals_list ~ "Grains",
  ...
  mutate(consumed_kg = yield_kg * (1-(`Total weight loss all levels`/100)),
         consumed_N_kg = case_when(StatCan_name %in% c("Triticale", "Wheat, all") ~ consumed_kg * 1.744/100, #avg wheat flour Lasaletta
                                   StatCan_name == "Corn for grain" ~ consumed_kg * 1.344/100, #maize flour
                                   StatCan_name == "Rye, all" ~ consumed_kg * 1.44/100, #rye flour
                                   StatCan_name == "Oats" ~ consumed_kg * 2.56/100, #rolled oats
                                   StatCan_name == "Soybeans" ~ consumed_kg * 6.08/100, # soybeans
                                   StatCan_name == "Beans, all dry (white and coloured)" ~ consumed_kg * 3.536/100, #beans, dry
                                   StatCan_name == "Chick peas" ~ consumed_kg * 3.216/100, 
                                   StatCan_name == "Lentils" ~ consumed_kg * 3.872/100,
                                   StatCan_name == "Peas, dry" ~ consumed_kg * 3.6/100,
                                   StatCan_name %in% oilseed_list ~ 0,
                                   StatCan_name %in% root_list ~ consumed_kg * (1-(`H20 content (%w/w)`/100)) * (`N conc prod (%DM)`/100),
                                   StatCan_name %in% fruit_list ~ consumed_kg * (1-(`H20 content (%w/w)`/100)) * (`N conc prod (%DM)`/100),
                                   StatCan_name %in% vegetable_list ~ consumed_kg * (1-(`H20 content (%w/w)`/100)) * (`N conc prod (%DM)`/100)),
...
cattle_pop <- read_csv("Data/Food/Production/Animal/32100130.csv")%>%
  as_tibble()%>%
...
pork_pop <- read_csv("Data/Food/Production/Animal/32100160.csv")%>%
  ...
  summarise(VALUE = mean(VALUE))%>%
  replace_na(list(VALUE = 0))
#didn't include under 7kg, 7 to 22 kilograms, because piglets feed hard to measure (also not sure how much of the population is slaughtered) 
...
N_animal_process <- N_manure_loss%>%
  ...
  mutate(slaughter_loss_N_kg = case_when(key_product %in% c("Pork", "Beef", "Chicken") ~ (N_animal_content_kg * animals_slaughtered) - yield_N_kg,
         ...
         consumed_N_kg = case_when(key_product == "Pork" ~ consumed_kg * 0.0448, #tenderloin CNF
                                   key_product == "Beef" ~ consumed_kg * 0.049, #ground beef CNF 
                                   key_product == "Chicken" ~ consumed_kg * 0.034, #ground chicken CNF
                                key_product == "Milk" ~ consumed_kg * 0.00512, #from CNF pdf (milk 2%)
...
Atlantic Salmon
Excretion  is  approximately 32 g N / kg of growth. FCR 1.1. Chapter One: Nutrient Releases from Salmon Aquaculture.
assume 70% of feed is soy/legume. https://www.aquaculture.ca/canadian-farmed-salmon 
...
fish_prod <- read_csv("Data/Food/Production/Fish/32100107.csv")%>%
  ...
  mutate(feed_kg = yeild_kg * 1.1,
         VNF_feed = feed_kg * 0.0011, #(VNF kg/kg for soy) 
         N_manure_kg = yeild_kg * 0.032,
         ...
         `VNF kgN/kg` = (N_manure_kg + N_waste_loss)/consumed_kg,
         `Food group` = "Fish",
         `N footprint sector` = "VN from meat")
...
VNF_animal_CAN <- bind_rows(VNF_fish, VNF_meat_CAN)
...
(NCI as more updated as compared to SIDE)
...
This is the general approach taken by the Canadian government in estimating GHG emissions from wastewater treatment 21
