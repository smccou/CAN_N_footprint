...
We draw strongly on the work down for the  N-calculator model developed by @leachnitrogen2012. We divide our nitrogen footprint model into the following sectors and sub sectors 1) Nr release due to burning fossil fuels, further divided for energy production, transport, oil and gas industries and other sources and 2) Nr related to food, divided into crop-related food production, livestock production and wastewater treatment. Our initial estimate is for the year 2015, the most recent year were data sources could be found for all relevant categories. Calculations and data sources for each sector are described in the following sections. 
...
## Energy
To estimate energy-related Nr we use a top-down approach summing Nr emissions estimates from provincial greenhouse gas and air pollutant inventories, to account for N~2~O and NO~x~, respectively.  
...
N_energy <- bind_rows(APEI_energy, GHG_energy)%>%
  filter(Source %in% c("Commercial/Residential/Institutional", "Construction and Manufacturing", "Electricity Generation", "Oil and Gas Industry", "Transport"), Region %in% c("Canada", "Newfoundland and Labrador",	"Prince Edward Island",	"Nova Scotia", "New Brunswick", "Quebec", 	"Ontario",	"Manitoba", "Saskatchewan", "Alberta", "British Columbia"))%>%
  group_by(Region, Source)%>%
...
## Food
For food-related Nr we  have used a hybrid top-down and bottom-up approach to estimate provincial N-footprints. We use a top-down approach to estimate provincial VNF values (further explained below) and then multiply these values by per capita food consumption data. 

```{r food_categorization, echo=FALSE, message = FALSE}
...
#Food grouping lists
cereals_list = c("Buckwheat", "Corn for grain", "Rye, all", "Oats", "Triticale", "Wheat, all") #stuff that people eat crops with less than 60% livestock allocation based on Karimi et al 2020 (Table A4)

feed_list = c("Barley", "Corn for silage", "Faba beans", "Sugar beets", "Tame hay") #stuff animal eat, crops with more than 60% livestock allocation based on Karimi et al 2020 (Table A4)
...
root_list = c("Potaoes", "carrots", "dry onions", "parsnips", "sweet potatoes", "rutabagas and turnips", "beets")

vegetable_list = c("cabbage", "cauliflowers", "sweet corn", "lettuce", "green and wax beans", "cucumbers and fresh gherkins (all varieties)", "tomatoes", "green peas", "asparagus", "celery", "spinach", "peppers", "radishes ", "garlic", "rhubarb", "broccoli", "Brussels sprouts", "leeks", "pumpkins", "French shallots and green onions", "squash and zucchini", "Asian radishes", "kale", "eggplants (except Chinese eggplants)", "radishes (except Asian radishes)")

food_crop_list = c("Buckwheat", "Corn for grain", "Rye, all", "Oats", "Triticale", "Wheat, all", "Beans, all dry (white and coloured)", "Chick peas", "Lentils", "Soybeans", "Peas, dry", "Canola (rapeseed)", "Sunflower seed", "Potaoes", "carrots", "dry onions", "parsnips", "sweet potatoes", "rutabagas and turnips", "beets", "cabbage", "cauliflowers", "sweet corn", "lettuce", "green and wax beans", "cucumbers and fresh gherkins (all varieties)", "tomatoes", "green peas", "asparagus", "celery", "spinach", "peppers", "radishes ", "garlic", "rhubarb", "broccoli", "Brussels sprouts", "leeks", "pumpkins", "French shallots and green onions", "squash and zucchini", "Asian radishes", "kale", "eggplants (except Chinese eggplants)", "radishes (except Asian radishes)", "apples", "blueberries", "grapes", "peaches", "pears", "plums and prune plums", "raspberries", "strawberries", "nectarines", "apricots", "sour cherries", "sweet cherries", "cranberries", "saskatoon berries", "blackberries", "currants", "haskaps")

all_crop_list = c("Buckwheat", "Corn for grain", "Rye, all", "Oats", "Triticale", "Wheat, all", "Beans, all dry (white and coloured)", "Chick peas", "Lentils", "Soybeans", "Peas, dry", "Canola (rapeseed)", "Sunflower seed", "Potaoes", "carrots", "dry onions", "parsnips", "sweet potatoes", "rutabagas and turnips", "beets", "cabbage", "cauliflowers", "sweet corn", "lettuce", "green and wax beans", "cucumbers and fresh gherkins (all varieties)", "tomatoes", "green peas", "asparagus", "celery", "spinach", "peppers", "radishes ", "garlic", "rhubarb", "broccoli", "Brussels sprouts", "leeks", "pumpkins", "French shallots and green onions", "squash and zucchini", "Asian radishes", "kale", "eggplants (except Chinese eggplants)", "radishes (except Asian radishes)", "apples", "blueberries", "grapes", "peaches", "pears", "plums and prune plums", "raspberries", "strawberries", "nectarines", "apricots", "sour cherries", "sweet cherries", "cranberries", "saskatoon berries", "blackberries", "currants", "haskaps", "Barley", "Corn for silage", "Faba beans", "Sugar beets", "Tame hay")
...
## Crop VNFs 
Virtual nitrogen factors estimate the N~r~ lost during the production of a food item (note that N~r~ loss during wastewater treatment is inlcuded in a seperate section of the N footprint), per total weight of food item see \@ref(eq:VN kg). 

\begin{equation}
VNF = kg N_{loss}/ kg_{food} (\#eq:VN kg)
\end{equation}

For food crops total N~r~ losses to the environment include the N~r~ loss on the field, procesessing and retail losses, and household waste , see \@ref(eq:N losses) 

\begin{equation}
N_{losstotalcrop} = N_{field loss} + N_{process loss} + N_{house loss} (\#eq: crop N losses)
\end{equation}

At each of these steps we estimated the difference between N inputs and N outputs as well as N~2~ denitrification. We include N~2~ denitrification because it is not a form of N~r~ and therefore is not included in our N footprint.

### Nitrogen loss at the field
To estimate N~r~ release that occurs at the field level we calculated the difference between N inputs of fertilizer and biological nitrogen fixation (BNF), N~2~ losses, and the output N contained in crop harvests \@ref(eq:N field loss). Manure is not included in this step as it is 'recycled' and not new N~r~. Manure N and related losses are dealt with in the animal VNF calculations.

\begin{equation}
N_{field loss} = N_{fert} + N_{BNF} +N_{residue recycling} - N_{denitrification}  (\#eq:N field loss)
\end{equation}

To estimate fertilizer N inputs we used provincial statistics on N fertilizer sales @governmentofcanadaFertilizer2020, and dissagregated the data based on crop areas in each province (based on various crop production surveys, see Appendix ...), and the provincial recommended N application rate for that crop @yangDevelopment2007. Because we cannot account for all crops grown in all provinces, the amount of N applied to each crop is likely an overestimation.  
...
#Note for fertilizer rates and sales data, PEI, NB NFLD and NS were grouped together as the Atlantic Provinces, so the apllication rate for these provinces was adjusted by this region 
N_rec_fert_crop <- mutate(crop_area, key_rec_fert = case_when(StatCan_name %in% vegetable_list ~ "Vegetables",
                                                              StatCan_name %in% c("carrots", "dry onions", "parsnips", "sweet potatoes", "rutabagas and turnips", "beets") ~ "Vegetables",
...
HI_Nconc_BNF <- read_excel("Data/Food/Production/Crop/HI_Nconc_BNF.xlsx")%>%
  as_tibble()
...
N_BNF <- mutate(crop_prod, key = case_when(StatCan_name %in% vegetable_list ~ "Vegetables",
                                                          ...
                                                          StatCan_name %in% c("apples", "peaches", "pears", "plums and prune plums", "nectarines", "apricots", "sour cherries", "sweet cherries") ~ "Tree fruit",
                                                          TRUE ~ StatCan_name))%>%
...
Denitrification losses from agricultural soils consist mainly of N2, but also include N2O and NOx (NO and NO2). Estimates remain uncertain due to methodological constraints and high spatiotemporal variability For overall denitrification loss in 2016 we used the approach of Janzen et al. (2003), assuming that 10 % of the soluble N contained in fertilizer was lost through denitrification and 10% of remaining organic N in soil is denitrified @karimiupdated2020. Assume 1:1 N20 to N2 @yangDevelopment2007. Could be 1:10 10:1. Sensitivity analysis? We also included residue recycling based on allocations of residues by @karimiupdated2020 
...
N_field_loss <- left_join(N_adjusted_fert, N_BNF, by = c("GEO"="GEO", "StatCan_name"="StatCan_name"))%>%
  ...
  mutate(residue_recycled_N_kg = residue_N_kg * `Residue: Soil %`,
         denitrification_N_kg = ((residue_N_kg * `Residue: Soil %` * 0.05) + (N_applied_crop_kg * 0.05)),
         N_loss_field_kg = total_new_N_kg - yield_N_kg + residue_recycled_N_kg - denitrification_N_kg
  )%>%
...
As provincial data for food processing and waste losses is currently unavaible in Canada, fractions lost in each food group were taken from the USDA loss adjusted food availability tables.  
...
N_crop_process <- mutate(N_field_loss, key_process = case_when(StatCan_name %in% cereals_list ~ "Grains",
  ...
  mutate(consumed_N_kg = yield_N_kg * `Total loss all levels`/100,
         consumed_kg = yield_kg * `Total loss all levels`/100,
...
VNF_food_group_prov <- N_crop_process%>%
...
VNF_food_group <- bind_rows(VNF_food_group_prov, VNF_food_group_CAN)
#write_csv(VNF_food_group, file.path("Results_Images/VNF_food_group.csv"))

ggplot(VNF_food_group, aes(fill = `Food group`, y = `VNF kgN/kg`, x = `Food group`))+
...
VNFs for meat and animal products(dairy and eggs) are calculated the same way as for crops (the total N~r~ lost during the production of a food item divided by weight of food item see \@ref(eq:VN kg)). However, total N~r~ released during the production of animal products includes animal feed , processing and waste losses, as well as the N~r~ released from manure \@ref(eq:animal N losses). We recognize that there is a slight overlap in how we might attribute emissions to animal products and meat. For example layer poultry produce both eggs and meat. However, in a previous study examining ammonia emissions in Canadian livestock (@sheppardLinkage2015), they found that only 3% of dairy emissions and 7% of layer emissions were attributed to meat production. Therefore we will estimate beef VNFs using only beef cattle, dairy emission from dairy cattle, chicken meat from broiler chickens, and eggs from layers.  

\begin{equation}
N_{losstotalanimal} = N_{feed} + N_{manure} + N_{process} (\#eq:animal N losses)
\end{equation}
...
#note on chicken numbers: there's no info # of broiler chickens. Have # of layers, and total number slaughtered. Therefore we have taken the number of total chickens slaughtered, and subtracted the total number of layers present for that year (since layers have ~1 year lifespan), to estimate number of broilers. 
...
#Sheppard 2015, Table 3
#adjust StatCan feedcrops to match sheppards categories (not quite the same)
#calf values are averaged between dairy and beef calves
feed_ratio <- read_excel("Data/Food/Production/Animal/Feed_ratios.xlsx")%>%
  ...
  mutate(feed_group = case_when(Region == "%Grain east" ~ "Barley",
                                Region == "%Grain west" ~ "Barley",
                                Region == "%Whole corn east" ~ "Corn for silage",
                                Region == "%Whole corn west" ~ "Corn for silage",
                                Region == "%Straw/Hay east" ~ "Tame hay",
                                Region == "%Straw/Hay west" ~ "Tame hay",
                                Region == "%Forage east" ~ "Pasture (nat)",
                                Region == "%Forage west" ~ "Pasture (nat)"))%>%
  left_join(HI_Nconc_BNF, by = c("feed_group" = "Crop")) #from @Karimi2020
...
VN_feed <- animal_pop%>%
  ...
  #kg feed
  mutate(feed_kg = case_when((GEO %in% c('British Columbia', 'Alberta', 'Saskatchewan', 'Manitoba')) & (Region %in% c("%Grain west", "%Whole corn west", "%Straw/Hay west", "%Forage west")) ~ (VALUE * 1000 * `Feed DMI kg/animal/year` / (1 - (`H20 content (%w/w)` / 100)) * Fraction),
                               (GEO %in% c("Ontario", "Quebec", "Nova Scotia", "Prince Edward Island", "New Brunswick", "Newfoundland and Labrador")) & (Region %in% c("%Grain east", "%Whole corn east", "%Straw/Hay east", "%Forage east")) ~ (VALUE * 1000 * `Feed DMI kg/animal/year` / (1 - (`H20 content (%w/w)` / 100)) * Fraction)))%>%
  left_join(VNF_feed_CAN, by = c("feed_group" = "StatCan_name"))%>%
  #VN from feed by feed type from avg_VNF_feed above
  mutate(VN_feed_kg = (feed_kg * `VNF kgN/kg`))%>%
  replace_na(list(VN_feed_kg=0))%>%
  group_by(GEO, key_feed)%>%
...
1) manure N production/year: 
feed N - animal N

feed N = feed rates/types/animal (@sheppardLinkage2015)  * Nconc (@karimiupdated2020)

animal N = animal population * slaughtered Fraction of population (based on census) slaughtered annually (@sheppardLinkage2015)  * animal weight at slaughter (@sheppardLinkage2015)  * total Nconc:(@karimiupdated2020) (CROSS CHECK WITH NATIONAL STATS)

2) manure storage loss: @huffmanEstimation2008

3) Denitrification: 

check HOLOS for provinces/ecoregions 

FAO 006  Guidelines,  Vol 4,  Table  11.3  p.11.24 https://www.ipcc-nggip.iges.or.jp/public/2006gl/pdf/4_Volume4/V4_11_Ch11_N2O&CO2.pdf 
fraction values: 0.05 - 0.5 NH3–N + NOx–N

5) Leaching: RSN values % for provinces: @deYong2009

4) Remaining is 'potential recycled manure N"

```{r N_manure, echo=FALSE, message = FALSE}

#Huffman 2008 manure rate & % on pasture
...
### Meat and animal products processing and waste
N losses during meat processing were estimated difference between N content of animals slaughtered (from @leachnitrogen2012) and N content of meat (N contents of meat from @FAO/Lassaletta).#Only partial data available for animals slaughtered at the provincial or national scale (available for chicken, but not pork or beef, which are only available at the national scale and only as total animal), 
...
N losses during meat processing were estimated using N content of live animals from @leachnitrogen2012, average processing losses from @sheppardLinkage2015, and N contents of meat from @FAO/Lassaletta.
...
food_consum_g <- food_consum_nci_raw%>%
  ...
  filter(value != 0) #otherwise too low?
...
aov_food <- (aov(BNSD25D ~ as.factor(GEO_PRV), data = food_consum_nci_raw)) 
summary(aov_food)
...
VNF_food_group_CAN <- read_csv("Results_Images/VNF_food_group_CAN.csv")

VN_food <- left_join(food_consum_g, VNF_food_group_CAN, by =c("food_group"="Food group"))%>%
  rename("GEO"="GEO.x")%>%
  mutate(`N kg/cap/year` = `consumed g/cap/year`/1000 * `VNF kgN/kg`,
         `N kg/year` = `consumed g/year`/1000 * `VNF kgN/kg`)%>%
  select(GEO, food_group, `N kg/cap/year`, `N kg/year`)%>%
  mutate(`N footprint sector` = "VN from food")%>%
...
```{r N footprint}
N_footprint <- bind_rows(N_wastewater, N_energy)%>%
  bind_rows(VN_food)%>%
  mutate(GEO = fct_relevel(GEO, "Canada", "British Columbia", "Alberta", "Saskatchewan", "Manitoba", "Ontario", "Quebec", "New Brunswick", "Nova Scotia", "Prince Edward Island", "Newfoundland and Labrador"), 
         Subsector = fct_relevel(Subsector, "Legumes", "Oilseeds", "Grains", "Fruits", "Vegetables", "Roots", "Wastewater Treatment", "Commercial/Residential/Institutional", "Electricity Generation", "Oil and Gas Industry", "Transport"),
         `N footprint sector` = fct_relevel(`N footprint sector`, "VN from food", "Wastewater Treatment", "Fossil Fuels"))
