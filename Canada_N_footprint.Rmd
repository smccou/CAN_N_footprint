...
APEI_energy <- read_csv("Data/fossil_fuels/APEI_inventory.csv", skip = 1)%>%
  ...
  mutate(Source = case_when(Source == "Ore and Mineral Industries" ~ "Construction and other industry",
                            ...
                            Source == "Commercial / Residential / Institutional" ~ "Commercial/Residential",
...
N_energy <- bind_rows(APEI_energy, GHG_energy)%>%
  filter(Source %in% c("Electricity Generation", "Oil and Gas Industry", "Transport", "Commercial/Residential"), Region %in% c("Canada", "Newfoundland and Labrador",	"Prince Edward Island",	"Nova Scotia", "New Brunswick", "Quebec", 	"Ontario",	"Manitoba", "Saskatchewan", "Alberta", "British Columbia"))%>%
  group_by(Region, Source)%>%
...
#choose which version you want
  ...
  filter(`VN` == "VN CAN weighted"| is.na(`VN`))%>%
  group_by(GEO, `N footprint sector`, Subsector)%>%
