...
N_crop_process <- mutate(N_field_loss, key_process = case_when(StatCan_name %in% cereals_list ~ "Grains",
  ...
  mutate(consumed_kg = yield_kg * `Total weight loss all levels`/100,
         consumed_N_kg = case_when(StatCan_name %in% cereals_list ~ consumed_kg * 1.744/100, #wheat flour Lasaletta
                                   StatCan_name %in% oilseed_list ~ 0,
                                   StatCan_name %in% legume_list ~ consumed_kg * 4.9/100, #avg of soybean and lentils
...
milk_prod <- milk%>%
  ...
  summarise(VALUE = sum(VALUE))%>%
  filter(year %in% years,
         `Dairy distribution` == "Fluid purposes")%>%  
...
ggplot(subset(N_footprint, `N footprint sector` %in% c("VN from crops", "VN from meat")), aes(fill = `Subsector`, y = `N kg/cap/year`, x = reorder(GEO, -`N kg/cap/year`)))+
  geom_bar(stat="identity") +
  #scale_fill_manual(values = mycolors)+
  scale_fill_brewer(palette="Paired")+
  labs(caption = "Food nitrogen footprints (N kg/cap/year)")+
  scale_x_discrete(labels = wrap_format(10))+
  coord_flip()+
  theme_classic()+
  theme(
...
ggplot(subset(N_footprint, `N footprint sector` %in% c("VN from crops", "VN from meat")), aes(fill = `Subsector`, y = `N kg/cap/year`, x = GEO))+
  geom_bar(position = "fill", stat="identity") +
  #scale_fill_manual(values = mycolors)+
  scale_fill_brewer(palette="Paired")+
  labs(caption = "Provincial Nitrogen footprints (N kg/cap/year)")+
