...
@leachnitrogen2012
@shindoTopdown2017
@shibataNitrogen2017: more national case studies are needed.The VNFs are available for a few
major exporting countries, however, there are few countries that have developed their own VNFs. The development of VNFs for major exporting countries is particularly important for accurately assessing the N footprint of countries that import food.
...
## Energy
To estimate energy-related Nr we use a top-down approach summing Nr emissions estimates from provincial greenhouse gas (to account for N2O) and air pollutant (to account for NOx) inventories.  
```{r Energy, echo=FALSE, message = FALSE}
APEI_raw <- read_csv("Data/Energy_APEI_GHG/EN_APEI-Can-Prov_Terr.csv")
#Canada has only Grand total
...
APEI_energy <- as_tibble(APEI_raw)%>%
  ...
  mutate(N_kg = ((`NOX (t)` * 1000 * 14.0067 / 46.0055) + (`NH3 (t)` * 1000 * 14.0067 / 17.031)))%>%
...
GHG_energy <- as_tibble(GHG_raw)%>%
  ...
  mutate(N_kg = N2O *1000000*14.0067/44.013)%>%
  filter(Year %in% years)%>%
  group_by(Region, Source)%>%
  summarise(`N kg/year` = mean(N_kg))%>%
  left_join(prov_pop, by = c("Region"="GEO"))%>%
...
To estimate fertilizer N inputs we used provincial statistics on N fertilizer sales @governmentofcanadaFertilizer2020, and dissagregated the data based on crop area in each province, and the provincial recommended N application rate for that crop. 
...
#Processing, packaging and distribution waste values were taken from USDA (Used same N conc values @karimi2020 for consistency (Lassaletta's slightly different).
...
#Processing loss
N_crop_process <- mutate(N_field_loss, key_process = case_when(StatCan_name %in% grain_list ~ "Grains",
  ...
  mutate(retail_N_kg = yield_N_kg * (1-`Processing and packaging`),
         ...
         consumed_N_kg = yeild_N_kg * (1-(`Total loss all levels`/100)),
...
VNF_food_group_CAN <- N_crop_process%>%
  mutate(Total_N_loss_kg = N_loss_field_kg + N_loss_processing_waste_kg + N_loss_retail_waste_kg + N_loss_household_waste_kg,
         ...
         VNF_kgN_kgN = (Total_N_loss_kg / edible_N_kg))%>%
  group_by(food_group)%>%
  summarise(`VNF_kgN_kg` = weighted.mean(`VNF_kgN_kg`, yield_kg ))%>%
  mutate(GEO = "Canada")%>%
  drop_na()
...
feed N = feed rates/types/animal (@Sheppard2015)  * Nconc (@karimiupdated2020)

animal N = animal population * slaughtered Fraction of population (based on census) slaughtered annually (@Sheppard2015)  * animal weight at slaughter (@Sheppard2015)  * total Nconc:(@karimiupdated2020) (CROSS CHECK WITH NATIONAL STATS)
...
VNF_food_group_CAN <- read.csv("Results_Images/VNF_food_group_CAN.csv")

VN_food <- left_join(food_consum_g, VNF_food_group_CAN, by =c("food_group"="food_group"))%>%
  rename("GEO"="GEO.x")%>%
  mutate(`N kg/cap/year` = `consumed g/cap/year`/1000 * VNF_kgN_kg,
         `N kg/year` = `consumed g/year`/1000 * VNF_kgN_kg)%>%
