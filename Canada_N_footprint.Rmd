...
## Crop VNFs 
Virtual nitrogen factors estimate the N~r~ lost during the production of a food item, per total weight of food item or weight of N in the food item, see \@ref(eq:VN kg) and \@ref(eq:VN kgN). 
...
\begin{equation}
VNF = kg N_{loss}/ kg N_{food} (\#eq:VN kgN)
\end{equation}

N~r~ losses to the environment include the N~r~ loss on the field, procesessing and retail losses, and household waste (note that N~r~ loss during wastewater treatment is inlcuded in a seperate section of the N footprint), see \@ref(eq:N losses) 

\begin{equation}
N_{total loss} = N_{field loss} + N_{process loss} + N_{house loss} (\#eq:N losses)
\end{equation}
...
N_crop_process <- mutate(N_field_loss, key_process = case_when(StatCan_name %in% cereals_list ~ "Grains",
                                                 ...
                                                 StatCan_name %in% fruit_list ~ "Fruit",
...
N_footprint <- bind_rows(N_wastewater, N_energy)%>%
  ...
  mutate(GEO = fct_relevel(GEO, "Canada", "British Columbia", "Alberta", "Saskatchewan", "Manitoba", "Ontario", "Quebec", "New Brunswick", "Nova Scotia", "Prince Edward Island", "Newfoundland and Labrador"), 
         Subsector = fct_relevel(Subsector, "pulses", "oilseeds", "cereals", "fruits", "vegetables", "roots", "Wastewater Treatment", "Commercial/Residential/Institutional", "Electricity Generation", "Oil and Gas Industry", "Transport"),
