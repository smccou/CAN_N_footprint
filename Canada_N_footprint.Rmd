...
## Food
We used a hybrid top-down and bottom-up approach to estimate the food-related portion of provincial N-footprints. We use a top-down approach to estimate provincial virtual nitrogen factor (VNF) values (further explained below) and then multiply these values by per capita food consumption data. We estimated crop 
...
To estimate N~r~ release that occurs at the field level we calculated the difference between N inputs of fertilizer and biological nitrogen fixation (BNF), N~2~ losses, and the output N contained in crop harvests \@ref(eq:N field loss). Manure is not included in this step as it is recycled N and not new N~r~. Manure N and related losses are dealt with in the animal VNF calculations.
...
## Graphing ##
ggplot(VNF_feed_CAN, aes(y = `VNF kgN/kg`, x = StatCan_name, fill = StatCan_name)) +
  geom_bar(position="stack", stat="identity") +
...
beef_pop <- read_csv("Data/Food/Production/Animal/32100130.csv")%>%
  ...
  filter(`Survey date` == "At January 1",
         ...
         (StatCan_name %in% c("Bulls, 1 year and over", "Dairy cows", "Beef cows", "Heifers for dairy replacement", "Total beef heifers", "Steers, 1 year and over", "Calves, under 1 year")),
         REF_DATE %in% years)%>%
...
pork_pop <- read_csv("Data/Food/Production/Animal/32100160.csv")%>%
  as_tibble()%>% 
  rename("StatCan_name" = "Livestock")%>%
  filter(`Survey date` == "At January 1", StatCan_name %in% c("Boars, 6 months and over", "Sows and gilts, 6 months and over", "Over 81 kilograms"))%>% #didn't include under 23kg because piglets/grower feed hard to measure (also not sure how much of the popultion is slaughtered)
  group_by(GEO, StatCan_name, UOM, SCALAR_FACTOR)%>%
  summarise(VALUE = mean(VALUE))%>%
  replace_na(list(VALUE = 0))
...
VNF_feed <- VNF_feed_CAN%>%
  select(StatCan_name, "VNF kgN/kgN")%>%
  pivot_wider(names_from = StatCan_name, values_from = "VNF kgN/kgN")%>%
  mutate(feed_VN = "feed VN kgN/kgN")
...
VN_N_feed <- read_excel("Data/Food/Production/Animal/Feed_ratios.xlsx")%>%  #feed ratios from #Sheppard 2015, Table 3, calf values are avg
  ...
  mutate(feed_VN = "feed VN kgN/kgN", feed_N = "feed N conc DMI")%>%
  left_join(VNF_feed)%>%
  #adjust StatCan feedcrops to match Sheppards categories (not quite the same)
  mutate(feed_group_VNF_kgN_kgN = case_when(Region == "%Grain east" ~ (`Barley` + `Corn for silage` + `Soybeans`)/3,
...
N_feed <- animal_pop%>%
  ...
  mutate(key_feed = case_when(StatCan_name == "Over 81 kilograms" ~ "Grower",
  ...
  mutate(VN_feed_kg = case_when((GEO %in% c('British Columbia', 'Alberta', 'Saskatchewan', 'Manitoba')) & (Region %in% c("%Grain west", "%Whole corn west", "%Straw/Hay west", "%Forage west")) ~ VALUE * 1000 * `Feed DMI kg/animal/year` * Fraction * `feed_group_VNF_kgN_kgN` * feed_group_N_conc_DMI / 100,
                                (GEO %in% c("Ontario", "Quebec", "Nova Scotia", "Prince Edward Island", "New Brunswick", "Newfoundland and Labrador")) & (Region %in% c("%Grain east", "%Whole corn east", "%Straw/Hay east", "%Forage east")) ~ VALUE * 1000 * `Feed DMI kg/animal/year` * Fraction * `feed_group_VNF_kgN_kgN` * feed_group_N_conc_DMI / 100))%>%
...
N_manure_loss <- N_manure_prod%>%
  ...
  mutate(N_manure_to_soil_kg = ((N_manure_prod_kg*(`% on pasture`/100)) + (N_manure_prod_kg*(1-(`% on pasture`/100))*(`N% after storage`/100))),
         ...
         N_manure_remaining_kg = N_manure_to_soil_kg - N_manure_leaching_kg, 
         N_manure_N20_kg = N_manure_remaining_kg * 0.05,
         N_manure_N2_kg = N_manure_remaining_kg * 0.05,
...
egg_prod <- layer_prod%>%
  ...
  mutate(yield_N_kg = (`Disposition of eggs in shell sold for consumption [116111111]` + `Disposition of eggs in shell, leakers and rejects [116111311]`)  * 0.0172, #from Lasaletta hen egg in shell
...
N_meat_process <- N_manure_loss%>%
  mutate(animals_slaughtered = VALUE * 1000 * `Fraction Population slaughtered`)%>%
  #group_by(key_storage)%>%
  ...
  mutate(meat_type = case_when(StatCan_name == "All other hogs"  ~ "Pork",
                               ...
                               TRUE ~ StatCan_name))%>%
  mutate(meat_yield_N_kg = case_when(meat_type == "Beef" ~ animals_slaughtered * `Weight at slaughter (kg/animal)` * `% dressed/live` * `%carcass meat` * `protein content meat` * `% N meat protein`,
                                   meat_type == "Pork" ~ animals_slaughtered * `Weight at slaughter (kg/animal)` * `% dressed/live` * `%carcass meat` * `protein content meat` * `% N meat protein`,
                                   meat_type == "Chicken" ~ animals_slaughtered * `Weight at slaughter (kg/animal)` * `% dressed/live` * `%carcass meat` * `protein content meat` * `% N meat protein`),
         meat_yield_kg = case_when(meat_type == "Beef" ~ animals_slaughtered * `Weight at slaughter (kg/animal)` * `% dressed/live` * `%carcass meat`,
                                   meat_type == "Pork" ~ animals_slaughtered * `Weight at slaughter (kg/animal)` * `% dressed/live` * `%carcass meat`,
                                   meat_type == "Chicken" ~ animals_slaughtered * `Weight at slaughter (kg/animal)` * `% dressed/live` * `%carcass meat`),
         N_loss_processing_kg = N_animal_content_kg - meat_yield_N_kg,
         meat_retail_kg = meat_yield_kg * (1-(Retail/100)),
         meat_consumed_kg = meat_retail_kg * (1-(Consumer/100)),
         meat_waste_N_kg = meat_yield_N_kg - meat_consumed_N_kg)%>%
  rename("Subsector" = "meat_type")
 
```

```{r VNF_animal, echo=FALSE, message = FALSE}
VNF_meat <-  N_meat_process%>%
  filter(Subsector %in% c("Beef", "Pork", "Chicken"))%>%
  mutate(`VNF kgN/kg` = (VN_feed_kg + N_manure_loss_storage_kg + N_manure_leaching_kg + N_manure_N20_kg + N_loss_processing_kg + meat_waste_N_kg) / meat_consumed_kg,
         `VNF kgN/kgN` = (VN_feed_kg + N_manure_loss_storage_kg + N_manure_leaching_kg + N_manure_N20_kg + N_loss_processing_kg + meat_waste_N_kg) / meat_consumed_N_kg)%>%
  group_by(GEO, Subsector)%>%
  summarise(`VNF kgN/kg` = sum(`VNF kgN/kg`), `VNF kgN/kgN` = sum(`VNF kgN/kgN`)))%>%
