...
field_crop_list = c("Barley", "Beans, all dry (white and coloured)", "Buckwheat", "Canary seed", "Canola (rapeseed)", "Chick peas", "Corn for grain", "Corn for silage", "Faba beans", "Flaxseed", "Lentils", "Rye, all", "Oats", "Soybeans", "Sugar beets", "Oats", "Peas, dry", "Sunflower seed", "Tame hay", "Triticale", "Wheat, all")                                          
...
N_BNF <- mutate(crop_area, key = case_when(StatCan_name == "Tame hay" ~ "Hay",
                                           TRUE ~ StatCan_name))%>%
  left_join(BNF_rates, by = c("key" = "crop", "GEO" = "Province"))%>%
  mutate(BNF_N_kg = area_ha * BNF_kg_ha)%>%
  replace_na(list(BNF_N_kg = 0))%>%
  select(StatCan_name, GEO, area_ha, BNF_kg_ha, BNF_N_kg)
```
...
N_crop_yield_residue <- mutate(crop_prod, key_yield = case_when(StatCan_name == "Carrots" ~ "Vegetables",
  ...
  mutate(yield_kg_DM = yield_kg * (1 - (`H20 content (%w/w)`/100)),
         yield_N_kg = yield_kg_DM * (`N conc prod (%DM)`/100),
         #Residue N
         residue_kg = yield_kg_DM * (1 - `HI`),
  ...
  mutate(residue_N_soil_kg = residue_N_kg * `Residue: Soil %`,
         #Root N
         root_kg = yield_kg_DM * (`Below Ground Residue (roots) (% of DM)`/100),
         root_N_kg = root_kg * (`N conc root (%DM)`/100),
         root_N_soil_kg = root_N_kg * `Root: Soil %`)
  
#Field N loss
N_field_loss <- left_join(N_input, N_crop_yield_residue, by = c("StatCan_name" = "StatCan_name", "GEO"="GEO"))%>%
