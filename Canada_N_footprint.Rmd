...
#see GHG executive summary file for explanation of categories
...
N_recommended_rates <- read_csv("Data/Food/recommended_fertilizer.csv", skip = 2)%>%
...
HI_Nconc_BNF <- read_csv("Data/Food/HI_Nconc_BNF.csv", skip = 2)
...
residue_allocation <- read_csv("Data/Food/residue_allocation.csv", skip = 1)

#provincial leaching values
leaching <- read_excel("Data/Food/Leaching.xlsx", range = "A3:V14")%>% 
  select(`...1`, `mean...22`)%>%
...
N_field_loss <- left_join(N_adjusted_fert, N_BNF, by = c("GEO"="GEO", "StatCan_name"="StatCan_name"))%>%
  ...
  mutate(residue_to_soil_N_kg = residue_N_kg * `Residue: Soil %`,
         ...
         N_diff_appl_harvest_kg = N_applied_crop_kg + N_BNF_kg - yield_N_kg,  #if manure is being applied is not actually negative
...
#consumed N content
food_N_cont <-read_csv("Data/Food/N_cont_food.csv", skip = 2)
...
VN_N_feed <- read_excel("Data/Food/feed_ratios.xlsx", skip = 1)%>%
  as_tibble()%>%
...
slaughter_processing <- read_excel("Data/Food/animal_processing_conversion.xlsx", skip=1)%>%  
  select(`Animal`,`Fraction Population slaughtered`, `Weight at slaughter (kg/animal)`)
  
animal_N <- read_excel("Data/Food/animal_N.xlsx", skip =1)
...
N_manure_prod <- N_feed%>%
  ...
  mutate(animals_slaughtered = VALUE * 1000 * `Fraction Population slaughtered`,
         ...
         yield_N_kg = case_when(key_product %in% c("Pork", "Beef", "Chicken") ~ yield_kg * `% N carcass`,
                                key_product == "Milk" ~ yield_kg * 0.00528,   # %N from Karimi
...
#*manure sanity check* compare to Huffman(mine): manure production per head broiler 0.4 (0.21), layer 0.6 (0.74), dairy cow 122 (96), beef cattle 78 (69), hogs 9.9 (14) 

```

```{r, N manure recycling and loss, echo=FALSE, message = FALSE}
manure_pasture <- read_excel("Data/Food/manure_pasture.xlsx", skip=1)%>% 
  as_tibble()%>%
  pivot_longer(`British Columbia`:`Newfoundland and Labrador`, names_to = "Province", values_to = "% on pasture")%>%
  select(!(`Kg N / head / yr`))

storage_loss <- read_excel("Data/Food/manure_storage.xlsx", skip=1)%>%
  as_tibble()%>%
...
#write_csv(VNF_meat, "Results_Images/Animal_VNF.csv")
...
factor(VNF_animal$GEO,levels = c("Canada", "British Columbia", "Alberta", "Saskatchewan", "Manitoba", "Ontario", "Quebec", "New Brunswick", "Nova Scotia", "PEI", "NFLD"))

```

```{r Sensitivity Analysis ANIMALS, echo=FALSE, message = FALSE}

N_model <- lm(`VNF kgN/kg` ~ `% N total` + `% on pasture` + `N% after storage` + `leaching loss %N` + `Total weight loss all levels`, VNF_meat_prov)

N_model <- lm(`VNF kgN/kg` ~ VN_feed_kg + N_manure_loss_storage_kg + N_manure_leaching_kg + N_manure_N20_kg + slaughter_loss_N_kg + food_waste_N_kg, VNF_meat_prov)


summary(N_model)


VNF_sim = function(VN_feed_kg, N_manure_loss_storage_kg, N_manure_leaching_kg, N_manure_N20_kg, slaughter_loss_N_kg, food_waste_N_kg,  plot = F) {
  VNF = NULL # empty flexible vector container
  
    VNF = (VN_feed_kg + N_manure_loss_storage_kg + N_manure_leaching_kg + N_manure_N20_kg + slaughter_loss_N_kg + slaughter_loss_N_kg + food_waste_N_kg)
  
  if (plot) {
    plot(VNF, type = "l", pch = 15, xlab = "Year", ylab = "Abundance")
  }
  
  VNF
}

VNF_sim(1,2,3,4,5,6, T)
...
#sanity check adults should be getting ~3.2-3.8kg N/year/cap
  # group_by(GEO)%>%
...
write_csv(food_stats, "Data/foodstats.csv")
...
VNF_food_CAN <- bind_rows(VNF_animal_CAN, VNF_crop_group_CAN)

VN_food <- left_join(food_consum, VNF_food_CAN, by =c("food_group"="Food group"))%>%
  rename("GEO"="GEO.x")%>%
  mutate(`N kg/cap/year` = `adjusted consumed kg/cap/year` * `VNF kgN/kg`,
         `N kg/year` = `adjusted consumed kg` * `VNF kgN/kg`)%>%
...
VN_food_sensitivity <- left_join(food_consum, VNF_food_CAN, by =c("food_group"="Food group"))%>%
  left_join(USA_VNF, by =c("food_group"="Food Group"))%>%
  rename("GEO"="GEO.x")%>%
  mutate(`Food imported from USA` = case_when(`food_group` == "Oilseeds" ~ `adjusted consumed kg/cap/year` * `VNF kgN/kg weighted`,
                                TRUE ~ `adjusted consumed kg/cap/year` * `VNF kg N / kg food USA`),
         `Food from Canada, proportioal to production` = `adjusted consumed kg/cap/year` * `VNF kgN/kg weighted`,
         `Food from within province` = `adjusted consumed kg/cap/year` * `VNF kgN/kg`)%>%
  rename(Subsector = food_group)%>%
  select(GEO, `N footprint sector`, Subsector, `VNF USA`, `VNF CAN unweighted`, `VNF CAN weighted`)%>%
  pivot_longer(cols = starts_with("VNF"), names_to = "Adjustment", values_to = "N kg/cap/year")%>%
  filter(GEO == "Canada")%>%
  group_by(`N footprint sector`, Adjustment)%>%
  summarise(`N kg/cap/year` = sum(`N kg/cap/year`))

ggplot(VN_food_sensitivity, aes(fill = `N footprint sector`, y = `N kg/cap/year`, x = (reorder(`Adjustment` , -`N kg/cap/year`))))+
  geom_bar(stat="identity") + 
  geom_text(aes(label = round(`N kg/cap/year`, 1)), position = position_stack(vjust= 0.5))+
...
Nitrogen removal occurs in two ways: 

(1) biological: a) combined well known autotrophic nitrification (partial or complete) and heterotrophic denitrification. or Completely autotrophic. In that group all anammox based processes and autotrophic denitrification using hydrogen (experimental). These are tertiary treatment. 80-85% removal TN for annamox (Oleszkiewicz, 2005)

(2) physicochemical processes 1) those which target removal of dissolved nitrogen species like ammonia, nitrates or DON, and 2) those designed to reduce TN nitrogen by removing particulate and/or colloidal nitrogen. Ammonia not generally removed, nitrate Processes applied to reduce the suspended and colloidal forms of nitrogen are most commonly used as a tertiary treatment

In North America the influent nitrogen concentration in municipal wastewater are in the range of 25 to 50 mg/L, with an average concentration of 40 mg/L. 

In a biological nutrient removal wastewater treatment approximately 65% of the incoming nitrogen is removed through nitrification and denitrification process

sludge is typically burned or added to landfill in Canada (so still released as Nr). 
...
N_wastewater <-  read_csv("Data/Food/wastewater/wastewater.csv", skip = 1)%>%
  ...
  mutate(`%N_removal` = case_when(`Population by treatment type` == "No treatment" ~ 0,
                                  ...
                                  `Population by treatment type` == "Secondary treatment" ~ 0,
                                  `Population by treatment type` == "Secondary treatment with additional phosphorus removal" ~ 0,
...
N_wastewater_prov <- N_wastewater%>%
  ...
  mutate(`N wastewater loss kg/cap/year` = `adjusted consumed N kg/cap/year` * (percent_pop) * (1-`%N_removal`),
         `N wastewater loss kg/year` = `adjusted consumed N kg` * (percent_pop) * (1-`%N_removal`))%>%
  group_by(GEO)%>%
...
N_footprint <- bind_rows(N_wastewater_Total, N_energy)%>%
  ...
  mutate(GEO = case_when(GEO == "Prince Edward Island" ~ "PEI",
         ...
         Subsector = fct_relevel(Subsector, "Beef", "Pork", "Chicken", "Eggs", "Milk", "Fish", "Legumes", "Oilseeds", "Grains", "Fruits", "Vegetables", "Roots", "Wastewater Treatment", "Electricity Generation", "Oil and Gas Industry", "Transport"),
         `N footprint sector` = fct_relevel(`N footprint sector`, "VN from meat","VN from crops", "Wastewater Treatment", "Fossil Fuels"))%>%
  group_by(GEO, `N footprint sector`)%>%
...
```{r Scenarios, echo=FALSE, message = FALSE}

#Transport Scenario: 50% reduction in transport emissions
N_energy_T1 <- N_energy%>%
  mutate(`N kg/cap/year` = case_when(Subsector == "Transport" ~ `N kg/cap/year` * 0.5,
                                     TRUE ~ `N kg/cap/year`))

N_footprint_T1 <- bind_rows(N_wastewater_Total, N_energy_T1)%>%
  bind_rows(VN_food)%>%
  mutate(GEO = case_when(GEO == "Prince Edward Island" ~ "PEI",
                   GEO == "Newfoundland and Labrador" ~ "NFLD",
                   TRUE ~ GEO),
         ...
         `N footprint sector` = fct_relevel(`N footprint sector`, "VN from meat","VN from crops", "Wastewater Treatment", "Fossil Fuels"))%>%
  group_by(GEO, `N footprint sector`)%>%
  summarise(`T1` = sum(`N kg/cap/year`)) 

#Energy Scenario: 50% reduction in oil and gas industry emissions
N_energy_E1 <- N_energy%>%
  mutate(`N kg/cap/year` = case_when(Subsector == "Oil and Gas Industry" ~ `N kg/cap/year` * 0.5,
                                     TRUE ~ `N kg/cap/year`))

N_footprint_E1 <- bind_rows(N_wastewater_Total, N_energy_E1)%>%
  bind_rows(VN_food)%>%
  mutate(GEO = case_when(GEO == "Prince Edward Island" ~ "PEI",
                   GEO == "Newfoundland and Labrador" ~ "NFLD",
                   TRUE ~ GEO),
         ...
         `N footprint sector` = fct_relevel(`N footprint sector`, "VN from meat","VN from crops", "Wastewater Treatment", "Fossil Fuels"))%>%
  group_by(GEO, `N footprint sector`)%>%
  summarise(`E1` = sum(`N kg/cap/year`))

#Transport and Energy Scenarios
N_energy_E2 <- N_energy%>%
  mutate(`N kg/cap/year` = case_when(Subsector == "Oil and Gas Industry" ~ `N kg/cap/year` * 0.5,
                                     Subsector == "Transport" ~ `N kg/cap/year` * 0.5,
                                     TRUE ~ `N kg/cap/year`))


#Food Scenario: beef reduced 50% chicken increase 50%
food_consum_F1 <- food_consum%>%
  mutate(`adjusted consumed N kg/cap/year` = case_when(food_group == "Beef" ~ `adjusted consumed N kg/cap/year` * 0.5,
                                              food_group == "Chicken" ~ `adjusted consumed N kg/cap/year` * 1.5, 
                                              TRUE ~ `adjusted consumed N kg/cap/year`),
         `adjusted consumed kg/cap/year` = case_when(food_group == "Beef" ~ `adjusted consumed kg/cap/year` * 0.5,
                                              food_group == "Chicken" ~ `adjusted consumed kg/cap/year` * 1.5, 
                                              TRUE ~ `adjusted consumed kg/cap/year`))

VN_food_F1 <- left_join(food_consum_F1, VNF_food_CAN, by =c("food_group"="Food group"))%>%
  rename("GEO"="GEO.x")%>%
  mutate(`N kg/cap/year` = `adjusted consumed kg/cap/year` * `VNF kgN/kg`,
         `N kg/year` = `adjusted consumed kg` * `VNF kgN/kg`)%>%
  select(GEO, food_group, `N footprint sector`, `N kg/cap/year`, `N kg/year`)%>%
...
N_wastewater_prov_F1 <- N_wastewater%>%
  left_join(food_consum_F1)%>%
  filter(!(GEO %in% c("Canada", "Yukon")))%>% #Canada average currently includes the North, need to remove and re-calculate for comparison with provinces
  mutate(`N wastewater loss kg/cap/year` = `adjusted consumed N kg/cap/year` * (percent_pop) * (1-`%N_removal`),
         `N wastewater loss kg/year` = `adjusted consumed N kg` * (percent_pop) * (1-`%N_removal`))%>%
  group_by(GEO)%>%
...
N_wastewater_CAN_F1 <- N_wastewater_prov_F1%>%
  summarise(`N kg/cap/year` = weighted.mean(`N kg/cap/year`, Population), `N kg/year` = sum(`N kg/year`))%>%
  mutate(`N footprint sector` = "Wastewater Treatment", "Subsector" = "Wastewater Treatment", GEO = "Canada")

N_wastewater_Total_F1 <- bind_rows(N_wastewater_prov_F1, N_wastewater_CAN_F1)%>%
  select(!(Population))

N_footprint_F1 <- bind_rows(N_wastewater_Total_F1, N_energy)%>%
  bind_rows(VN_food_F1)%>%
  mutate(GEO = case_when(GEO == "Prince Edward Island" ~ "PEI",
                   GEO == "Newfoundland and Labrador" ~ "NFLD",
                   TRUE ~ GEO),
         GEO = fct_relevel(GEO, "Canada", "British Columbia", "Alberta", "Saskatchewan", "Manitoba", "Ontario", "Quebec", "New Brunswick", "Nova Scotia", "PEI", "NFLD"), 
         ...
         `N footprint sector` = fct_relevel(`N footprint sector`, "VN from meat","VN from crops", "Wastewater Treatment", "Fossil Fuels"))%>%
  group_by(GEO, `N footprint sector`)%>%
  summarise(`F1` = sum(`N kg/cap/year`)) 

#Improved Waste water treatment: 100% population tertiary treatment
N_wastewater_prov_W1 <- N_wastewater%>%
  left_join(food_consum)%>%
  filter(!(GEO %in% c("Canada", "Yukon")))%>% #Canada average currently includes the North, need to remove and re-calculate for comparison with provinces
  filter(`Population by treatment type` == "Tertiary treatment")%>%
  mutate(`N wastewater loss kg/cap/year` = `adjusted consumed N kg/cap/year` * 0.5)%>%
  group_by(GEO)%>%
  summarise(`N kg/cap/year` = sum(`N wastewater loss kg/cap/year`))%>%
  mutate(`N footprint sector` = "Wastewater Treatment", "Subsector" = "Wastewater Treatment")%>%
  left_join(prov_pop)
 
N_wastewater_CAN_W1 <- N_wastewater_prov_W1%>%
  summarise(`N kg/cap/year` = weighted.mean(`N kg/cap/year`, Population))%>%
  mutate(`N footprint sector` = "Wastewater Treatment", "Subsector" = "Wastewater Treatment", GEO = "Canada")

N_wastewater_Total_W1 <- bind_rows(N_wastewater_prov_W1, N_wastewater_CAN_W1)%>%
  select(!(Population))

N_footprint_W1 <- bind_rows(N_wastewater_Total_W1, N_energy)%>%
  bind_rows(VN_food)%>%
  mutate(GEO = case_when(GEO == "Prince Edward Island" ~ "PEI",
                   GEO == "Newfoundland and Labrador" ~ "NFLD",
                   TRUE ~ GEO),
         ...
         `N footprint sector` = fct_relevel(`N footprint sector`, "VN from meat","VN from crops", "Wastewater Treatment", "Fossil Fuels"))%>%
  group_by(GEO, `N footprint sector`)%>%
  summarise(`W1` = sum(`N kg/cap/year`)) 

#Lower Beef and better waste water treatment
N_wastewater_prov_W2 <- N_wastewater%>%
  left_join(food_consum_F1)%>%
  filter(!(GEO %in% c("Canada", "Yukon")))%>% #Canada average currently includes the North, need to remove and re-calculate for comparison with provinces
  filter(`Population by treatment type` == "Tertiary treatment")%>%
  mutate(`N wastewater loss kg/cap/year` = `adjusted consumed N kg/cap/year` * 0.5)%>%
...
N_wastewater_CAN_W2 <- N_wastewater_prov_W2%>%
  summarise(`N kg/cap/year` = weighted.mean(`N kg/cap/year`, Population))%>%
  mutate(`N footprint sector` = "Wastewater Treatment", "Subsector" = "Wastewater Treatment", GEO = "Canada")

N_wastewater_Total_W2 <- bind_rows(N_wastewater_prov_W2, N_wastewater_CAN_W2)%>%
  select(!(Population))
  

#All scenarios A3
N_footprint_A4 <- bind_rows(N_wastewater_Total_W2, N_energy_E2)%>%
  bind_rows(VN_food_F1)%>%
  mutate(GEO = case_when(GEO == "Prince Edward Island" ~ "PEI",
                   GEO == "Newfoundland and Labrador" ~ "NFLD",
                   TRUE ~ GEO),
         ...
         `N footprint sector` = fct_relevel(`N footprint sector`, "VN from meat","VN from crops", "Wastewater Treatment", "Fossil Fuels"))%>%
  group_by(GEO, `N footprint sector`)%>%
  summarise(`A4` = sum(`N kg/cap/year`)) 

scenario_comparison <- left_join(N_footprint, N_footprint_T1)%>%
  left_join(N_footprint_E1)%>%
  left_join(N_footprint_F1)%>%
  left_join(N_footprint_W1)%>%
  left_join(N_footprint_A4)%>%
  select(!`N kg/year`)%>%
  rename("O" = "N kg/cap/year" )%>%
  pivot_longer(cols = c("O", "T1", "E1", "W1", "F1", "A4"), names_to = "Scenario", values_to = "N kg/cap/year")%>%
  filter(GEO == "Canada")

ggplot(scenario_comparison, aes(fill = `N footprint sector`, y = `N kg/cap/year`, x = reorder(Scenario, + `N kg/cap/year`)))+
  geom_bar(stat="identity")+
  #facet_wrap(vars(GEO))+
  scale_fill_brewer(palette="Paired")+
  labs(x = "Scenario", y="kg N/cap/year" )+
  coord_flip()+
  theme_classic()+
  theme(
...
VNF_food_graph <- bind_rows(VNF_meat, VNF_crop_group)

ggplot(VNF_food_graph, aes(fill = `GEO`, y = `VNF kgN/kg`*1000, x = `GEO`))+
  geom_bar(position="stack", stat="identity") +
  facet_wrap(~factor(`Food group`, levels=c("Roots",  "Grains", "Legumes", "Fruits", "Vegetables", "Oilseeds","Milk", "Eggs", "Chicken", "Pork", "Beef" )), scales = "free")+
...
crop_VNF_graph <- ggplot(VNF_crop_group, aes(fill = `GEO`, y = `VNF kgN/kg`*1000, x = `GEO`))+
  geom_bar(position="stack", stat="identity") +
  facet_wrap(~factor(`Food group`, levels=c("Roots",  "Grains", "Legumes", "Fruits", "Vegetables", "Oilseeds" )), scales = "free_x")+
  #labs(caption = "Virtual nitrogen factors (g N / kg) for food crops")+
  #scale_y_log10()+
  coord_flip()+
  scale_fill_brewer(palette="Paired")+
...
meat_VNF_graph <- ggplot(VNF_meat, aes(fill = `GEO`, y = `VNF kgN/kg`*1000, x = GEO))+
  geom_bar(position="stack", stat="identity")+
  facet_wrap(~factor(`Food group`, levels=c("Milk", "Eggs", "Chicken", "Pork", "Beef")), scales = "free_x")+
  #facet_grid(~factor(`Food group`, levels=c("Eggs", "Chicken", "Pork", "Milk", "Beef")), scales= "free")+
...
ggarrange(crop_VNF_graph, meat_VNF_graph ,
                    labels = c("A","B"),
                    ncol = 2,
                    legend = "none")
          #common.legend = TRUE)

ggsave("Results_Images/VNFs.pdf")  
...
subsector_graph <- ggplot(N_footprint, aes(fill = `Subsector`, y = `N kg/cap/year`, x = reorder(GEO, -`N kg/cap/year`)))+
  geom_bar(stat="identity") +
...
ggsave("Results_Images/N_print_subsectors_graph.pdf")  
