...
broiler_pop <- poultry_prod%>%
  mutate(VALUE = case_when(GEO == "New Brunswick" ~ 53134 * 12/117,
                           GEO == "Prince Edward Island" ~ 53134 * 12/117, 
                           GEO == "Nova Scotia" ~ 53134 * 87/117,
                           GEO == "Newfoundland and Labrador" ~ 53134  * 6/117,
                           TRUE ~ VALUE))%>%
...
cattle_pop <- read_csv("Data/Food/Production/Animal/32100130.csv")%>%
  ...
  filter(`Survey date` == "At January 1",
         ...
         StatCan_name %in% c("Dairy cows", "Beef cows", "Total beef heifers", "Bulls, 1 year and over", "Steers, 1 year and over"),
         REF_DATE %in% years)%>%
...
#Did not include "Heifers for dairy replacement" "Calves, under 1 year"
...
pork_pop <- read_csv("Data/Food/Production/Animal/32100160.csv")%>%
  as_tibble()%>% 
  rename("StatCan_name" = "Livestock")%>%
  filter(`Survey date` == "At January 1", StatCan_name %in% c( "Over 81 kilograms", "54 to 80 kilograms", "23 to 53 kilograms", "Boars, 6 months and over", "Sows and gilts, 6 months and over"))%>% 
  group_by(GEO, StatCan_name, UOM, SCALAR_FACTOR)%>%
  summarise(VALUE = mean(VALUE))%>%
  replace_na(list(VALUE = 0))
#didn't include under 23kg because piglets/grower feed hard to measure (also not sure how much of the population is slaughtered) 
...
#*manure sanity check* compare to Huffman(mine): manure production per head broiler 0.4 (0.21), layer 0.6 (0.74), dairy cow 122 (96), beef cattle 78 (69), hogs 9.9 (14) Ontario is closer to this
...
N_footprint <- bind_rows(N_wastewater_Total, N_energy)%>%
  ...
  mutate(GEO = fct_relevel(GEO, "Canada", "British Columbia", "Alberta", "Saskatchewan", "Manitoba", "Ontario", "Quebec", "New Brunswick", "Nova Scotia", "Prince Edward Island", "Newfoundland and Labrador"), 
         Subsector = fct_relevel(Subsector, "Beef", "Pork", "Chicken", "Eggs", "Milk", "Legumes", "Oilseeds", "Grains", "Fruits", "Vegetables", "Roots", "Wastewater Treatment", "Electricity Generation", "Oil and Gas Industry", "Transport"),
...
VNF_crop_graph <- VNF_crop_group

ggplot(VNF_crop_group, aes(fill = `GEO`, y = `VNF g N/kg`, x = `GEO`))+
  geom_bar(position="stack", stat="identity") +
  facet_wrap(~factor(`Food group`, levels=c("Roots", "Fruits", "Grains", "Vegetables", "Oilseeds", "Legumes")), scales = "free_x")+
...
ggplot(VNF_meat, aes(fill = `GEO`, y = `VNF kgN/kg`*1000, x = GEO))+
  geom_bar(position="stack", stat="identity")+
  facet_wrap(~factor(`Food group`, levels=c("Milk", "Eggs", "Chicken", "Pork", "Beef")), scales = "free")+
...
ggplot(subset(N_footprint, GEO %in% c("Newfoundland and Labrador", "Prince Edward Island", "Nova Scotia", "New Brunswick", "Quebec", "Ontario", "Manitoba", "Saskatchewan", "Alberta", "British Columbia")), aes(fill = `N footprint sector`, y = `N kg/year`/1000000, x = reorder(GEO, - `N kg/year`)))+
  geom_bar(stat="identity") +
  scale_fill_brewer(palette="Paired")+
  scale_x_discrete(labels = wrap_format(10))+
  labs(y = "Total provincial N footprint (Gg/year)")+
  coord_flip()+
  theme_classic()+
  theme(
    ...
    axis.ticks = element_blank(),
    plot.caption = element_text(hjust = 0))
#ggsave("Results_Images/N_print_total.pdf")  

```

```{r Graph Provincial footprints per capita by sector, echo=FALSE, message = FALSE}

sector_graph <- ggplot(N_footprint, aes(fill = `N footprint sector`, y = `N kg/cap/year`, x = reorder(GEO, - `N kg/cap/year`)))+
  geom_bar(stat="identity") +
  scale_fill_brewer(palette="Paired")+
  labs(caption = "Provincial Nitrogen footprints by sector (N kg/cap/year)")+
  scale_x_discrete(labels = wrap_format(10))+
  coord_flip()+
  theme_classic()+
  theme(
...
```{r Graph Provincial footprints per capita by sector percentage, echo=FALSE, message = FALSE}

sector_percent_graph <- ggplot(N_footprint, aes(fill = `N footprint sector`, y = `N kg/cap/year`, x = reorder(GEO, - `N kg/cap/year`)))+
  geom_bar(position = "fill", stat="identity") +
  scale_fill_brewer(palette="Paired")+
  labs(caption = "Sector contribution to per capita provincial N footprints, % of total")+
  scale_x_discrete(labels = wrap_format(10))+
  coord_flip()+
  theme_classic()+
  theme(
    ...
    axis.ticks = element_blank(),
    plot.caption = element_text(hjust = 0))
#ggsave("Results_Images/N_print_sectors_cap_percent.pdf")
...
subsector_graph <- ggplot(N_footprint, aes(fill = `Subsector`, y = `N kg/cap/year`, x = reorder(GEO, -`N kg/cap/year`)))+
  ...
  #scale_fill_brewer(palette="Paired")+
  labs(caption = "Nitrogen footprints by subsector (N kg/cap/year)")+
  scale_x_discrete(labels = wrap_format(10))+
  coord_flip()+
  theme_classic()+
  theme(
    ...
    axis.ticks = element_blank(),
    plot.caption = element_text(hjust = 0))
#ggsave("Results_Images/N_print_food_subsectors_cap.pdf")

```

```{r Graph Provincial footprints per capita by subsector, % of total, echo=FALSE}
...
subsector_percent_graph <- ggplot(N_footprint, aes(fill = `Subsector`, y = `N kg/cap/year`, x = reorder(GEO, -`N kg/cap/year`)))+
  ...
  scale_fill_manual(values = mycolors)+
  #scale_fill_brewer(palette="Paired")+
  labs(caption = "Subsector contribution to per capita provincial N footprints, % of total")+
  coord_flip()+
  scale_x_discrete(labels = wrap_format(10))+
  ...
  theme(
    ...
    axis.ticks = element_blank(),
    plot.caption = element_text(hjust = 0))
#ggsave("Results_Images/N_print_subsectors_cap_percent.pdf")  

```

```{r, echo=FALSE, message = FALSE}

ggarrange(sector_graph, sector_percent_graph, subsector_graph, subsector_percent_graph,
                    labels = c("A","B","C","D"),
                    ncol = 2, nrow = 2,
          legend = "none")
ggsave("Results_Images/N_print_subsectors_cap_percent.pdf")  
