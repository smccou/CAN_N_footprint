...
leaching <- read_csv("Data/Food/food_production/Leaching.csv", skip=2)%>%
  select(`X1`, `mean_2`)%>%
  rename("GEO" = `X1`, "leaching loss %N" = `mean_2`)
...
food_processing_waste <- read_csv("Data/Food/food_loss_USDA.csv", skip = 1)
...
VN_N_feed <- read_csv("Data/Food/food_production/feed_ratios.csv", skip = 1)%>%
  as_tibble()%>%
...
slaughter_processing <- read_csv("Data/Food/food_production/animal_processing_conversion.csv", skip=1)%>%  
  select(`Animal`,`Fraction Population slaughtered`, `Weight at slaughter (kg/animal)`)
  
animal_N <- read_csv("Data/Food/food_production/animal_N.csv", skip =1)
...
manure_pasture <- read_csv("Data/Food/food_production/manure_pasture.csv", skip=1)%>% 
  as_tibble()%>%
  pivot_longer(`British Columbia`:`Newfoundland and Labrador`, names_to = "Province", values_to = "% on pasture")%>%
  select(!(`Kg N / head / yr`))

storage_loss <- read_csv("Data/Food/food_production/manure_storage.csv", skip=1)%>%
  as_tibble()%>%
...
CCHS_food_names <- read_csv("Data/Food/consumption/cchs-82M0024-E-2015-nu-nci_names.csv")%>%
...
USA_VNF <- read_csv("Data/Food/Leach_USA_VNF.csv", skip = 1)
...
ggsave("Results_Images/fig_5_VNF_graph.pdf", height = 10, width = 10)  
...
ggsave("Results_Images/fig_3_N_print_sectors_graph.pdf", height = 10, width = 10)  
...
subsector_percent_graph <- ggplot(N_footprint, aes(fill = `Subsector`, y = `N kg/cap/year`, x = reorder(GEO, -`N kg/cap/year`)))+
  ...
  scale_fill_manual(values = mycolors)+
  scale_y_continuous(expand=c(0,0)) +
  labs(caption = "Subsector, contribution, fraction of total")+
  coord_flip()+
...
ggsave("Results_Images/fig_4_N_print_subsectors_graph.pdf", height = 8, width = 10)  
...
```{r VN sensitivity graph, echo=FALSE, message = FALSE}
