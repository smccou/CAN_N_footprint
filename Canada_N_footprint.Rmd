...
VNF_food_crop_CAN <- N_crop_process%>%
  ...
  mutate(GEO = "Canada")%>%
  drop_na()
#write_csv(VNF_food_group, file.path("Results_Images/VNF_food_group_CAN.csv"))
...
aov_food <- (aov(BNSD25D ~ GEO_PRV, data = food_consum_nci_raw)) 
summary(aov_food)

VNF_food_group_CAN <- read.csv("Results_Images/VNF_food_group_CAN.csv")

VN_food <- left_join(food_consum_g, VNF_food_crop_CAN)%>%
  mutate(`N kg/cap/year` = `consumed g/cap/year`/1000 * VNF_kgN_kg )%>%
  select(GEO, food_group, `N kg/cap/year`)%>%
...
APEI_energy <- as_tibble(APEI_raw)%>%
  ...
  summarise(N_kg = mean(N_kg))

CAN_APEI <- APEI_energy%>%
  group_by(Source)%>%
  summarise(N_kg = sum(N_kg))%>%
  mutate(Region = "Canada")

APEI_energy <- bind_rows(APEI_energy, CAN_APEI)%>%
  left_join(prov_pop, by = c("Region"="GEO"))%>%
  mutate(`N kg/cap/year` = N_kg / Population)%>%
  select(Region, Source, `N kg/cap/year`)
...
N_footprint <- bind_rows(N_wastewater, N_energy)%>%
  bind_rows(VN_food)%>%
  mutate(GEO = fct_relevel(GEO, "Canada", "British Columbia", "Alberta", "Saskatchewan", "Manitoba", "Ontario", "Quebec", "New Brunswick", "Nova Scotia", "Prince Edward Island"), 
...
#ggsave("Results_Images/N_print_sectors.pdf")  
