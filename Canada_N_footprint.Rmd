...
broiler_pop <- left_join(poultry_prod,layer_pop, by = c("year" = "year","Province" = "Province", "SCALAR_FACTOR" = "SCALAR_FACTOR"))%>%
...
#Disposition of eggs in shell, leakers and rejects [116111311] for waste?

eggs <- eggs_raw%>%
  as_tibble()%>%
  filter(`Production and disposition` == "Disposition of eggs in shell sold for consumption [116111111]",
         REF_DATE %in% years)%>%
  mutate(quantity = VALUE * 12 * 1000)
