...
ggplot(VNF_meat, aes(fill = `GEO`, y = `VNF kgN/kg`*1000, x = `GEO`))+
  geom_bar(position="stack", stat="identity") +
  facet_wrap(~factor(`Food group`, levels=c("Beef", "Milk", "Pork", "Chicken", "Eggs")))+
...
food_consum_g_CAN_noO <- food_consum_g%>%
  group_by(food_group)%>%
  filter(value > 0)%>%
...
#use national availability stats and adjust them by CCHS data
...
ggplot(data_percentage, aes(fill = `N footprint sector`, y = `N kg/cap/year`, x = GEO))+
...
```{r Nitrogen footprint subsectors, echo=FALSE, message = FALSE}
colors <- 15
